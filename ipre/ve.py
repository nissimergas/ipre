...
class  MiMapa (QtGui.QWidget):
    ...
    def generar(self):
        ...
        for ball in self.balls:
            ...
            code+="(BALL ball{0})(at-ball_x ball{0} {1})(at-ball_y ball{0} {2}) ".format(id, "x"+str(int(ball.x)), "y"+str(int(ball.y)))
            id+=1
       ...
       # for x in range(16):
        ...
        code+="(at-robby_x {0}) (at-robby_y {1}) ".format("x"+str(int(self.robots[0].x)), "y"+str(int(self.robots[0].y)))
        code+=")" #cierre init
        code+="""(:goal (and"""
        code+="(at-robby_x x{0}) (at-robby_y y{1})".format(str(int(self.robots_final[0].x-17)),str(int(self.robots_final[0].y)))
        ...
        for ball in self.balls_final:
            code+=" (at-ball_x ball{0} {1}) (at-ball_y ball{0} {2}) ".format(id, "x"+str(int(ball.x-17)), "y"+str(int(ball.y)))
            id+=1
    ...
    """def move(self, x1,y1,x2,y2):
        x1_real=int(x1[1:])*37+22
        y1_real=int(y1[1:])*33+141
       # if self.robots[0].x==int(x1[1:]):
        #    if self.robots[0].y==int(y1[1:]):
        x2_real=int(x2[1:])*37+22
        y2_real=int(y2[1:])*33+141

        #self.robots[0].pic.move(x2_real,y2_real)


        #sleep(0.1)
        self.robots[0].x=x2_real
        self.robots[0].y=y2_real
        ...
        #self.timer.start(65)
        #self.robots[0].mover(x2_real,y2_real)
        self._timer = QTimer(interval=1,
                                    timeout=self.mover)
        self._timer.start(2022)"""
